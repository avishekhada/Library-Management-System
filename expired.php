...
{
  $var1='<p style="color:white; background-color:green;">Return</p>';
  mysqli_query($db,"update `issue` set approve='$var1' where username='$_POST[username]'  and id='$_POST[id]' ");
}
}
...
?>
    ...
    <?php
     if(isset($_SESSION['login_user']))
     {
      ...
      $sql="SELECT student.username,firstname, books.id,Title,Author,approve,issue,issue.return FROM student inner join issue ON 
      student.username=issue.username inner join books
       ON issue.id=books.id WHERE issue.approve !=''  and issue.approve !='yes'   ORDER BY `issue`.`return` ASC";
